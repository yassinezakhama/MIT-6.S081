...
sys_sigalarm(void)
{
  ...
  if (argint(0, &alarmticks) < 0 || argaddr(1, &alarmhandler) < 0) {
    return -1;
  }

  struct proc *p = myproc();
  p->alarm->ticks = alarmticks;
  p->alarm->remticks = alarmticks;
  p->alarm->handler = (void (*)())alarmhandler;
  return 0;
}
...
sys_sigreturn(void)
{
  ...
  p->alarm->remticks = p->alarm->ticks;  
  return 0;
}
